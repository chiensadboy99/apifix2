...
class AdvancedDeepLearningAI {
    ...
    calculateTrueConfidence(predictions) {
        ...
        const winningPrediction = votes.T > votes.X ? 'T' : (votes.X > votes.T ? 'X' : null);
        if (!winningPrediction) return 0.5;
        const winningVotes = Math.max(votes.T, votes.X);
    ...
    predict() {
        ...
        let finalPrediction = votes.T >= votes.X ? 'T' : 'X';
        const confidence = this.calculateTrueConfidence(predictions);
        return { prediction: finalPrediction === 'T' ? 'tài' : 'xỉu', confidence: confidence, rawPrediction: finalPrediction, algorithms: predictions.length };
    }
    addResult(record) {
        const parsed = {
    ...
    getPattern() {
        ...
        return { recent: recent, long: long, discovered: this.discoverDominantPattern(tx.slice(-30)) };
    }
...
app.get("/api/taixiu/sunwin", async (request, reply) => {
    try {
        ...
        if (!lastResult) return { id: "NT Sunwin AI", status: "đang chờ dữ liệu phiên đầu tiên...", phien_truoc: null, tong: null, ket_qua: "đang chờ...", pattern_gan_nhat: pattern.recent, pattern_dai: pattern.long, phien_hien_tai: null, du_doan: "đang tính...", do_tin_cay_ai: "50%" };
        return { id: "NT Sunwin AI", phien_truoc: lastResult.session, xuc_xac: lastResult.dice, tong: lastResult.total, ket_qua: lastResult.result.toLowerCase(), pattern_gan_nhat: pattern.recent, pattern_dai: pattern.long, pattern_chu_dao: pattern.discovered, phien_hien_tai: lastResult.session + 1, du_doan: currentPrediction.prediction, do_tin_cay_ai: `${(currentPrediction.confidence * 100).toFixed(1)}%` };
    } catch (error) { return { id: "NT Sunwin AI", error: "Hệ thống đang xử lý lỗi hoặc chưa đủ dữ liệu." }; }
});
...
app.get("/api/taixiu/history", async () => {
    try {
        ...
        return valid.slice(0, 30).map((i) => ({ session: i.session, dice: i.dice, total: i.total, result: i.result.toLowerCase(), tx: i.total >= 11 ? 'T' : 'X' }));
    } catch (e) { return { message: "lỗi hệ thống" }; }
});
...
app.get("/api/taixiu/ai-stats", async () => {
    try {
        ...
        const pattern = ai.getPattern();
        return { status: "online", ai_version: "9.0 - Ultra Pattern Recognition", current_prediction: prediction.prediction, confidence: `${(prediction.confidence * 100).toFixed(1)}%`, algorithms_active: prediction.algorithms, pattern_dominant: pattern.discovered, algorithm_stats: stats };
    } catch (e) { return { error: "Lỗi hệ thống" }; }
});

app.get("/", async () => {
    return { status: "online", name: "NT Sunwin AI", version: "9.0 - Ultra Pattern Recognition & Quantum AI", description: "Hệ thống AI dự đoán với 100+ mẫu cầu và học máy nâng cao", algorithms_count: ALGORITHMS.length, pattern_database: Object.keys(PATTERN_DATABASE).length + " mẫu cầu", features: ["Ultra Pattern Recognition (100+ mẫu)", "Quantum Adaptive AI", "Smart Bridge Detection", "Real-time Adaptive Learning", "Multi-layer Pattern Fusion"] };
});

const start = async () => {
    try {
        await app.listen({ port: PORT, host: "0.0.0.0" });
    } catch (err) { console.error('❌ Lỗi khởi động server:', err); process.exit(1); }
};
...
function connectRikWebSocket() {
    if (rikWS && (rikWS.readyState === WebSocket.OPEN || rikWS.readyState === WebSocket.CONNECTING)) rikWS.close();
    clearInterval(rikIntervalCmd);
    try { rikWS = new WebSocket(`${WS_URL}${TOKEN}`); } catch (e) { setTimeout(connectRikWebSocket, 5000); return; }
    rikWS.on("open", () => {
        const authPayload = [1, "MiniGame", "SC_giathinh2133", "thinh211", { info: JSON.stringify({ ipAddress: "2402:800:62cd:b4d1:8c64:a3c9:12bf:c19a", wsToken: TOKEN, userId: "cdbaf598-e4ef-47f8-b4a6-a4881098db86", username: "SC_hellokietne212", timestamp: Date.now() }), signature: "", pid: 5, subi: true }];
    ...
    rikWS.on("message", (data) => {
        try {
            ...
            if (json.session && Array.isArray(json.dice)) {
                const record = { session: json.session, dice: json.dice, total: json.total, result: json.result };
                ...
                const newHistory = json[1].htr.map((i) => ({ session: i.sid, dice: [i.d1, i.d2, i.d3], total: i.d1 + i.d2 + i.d3, result: i.d1 + i.d2 + i.d3 >= 11 ? "Tài" : "Xỉu" })).sort((a, b) => a.session - b.session);
                ai.loadHistory(newHistory);
                rikResults = newHistory.slice(-50).sort((a, b) => b.session - a.session);
            }
        } catch (e) {}
    });
    rikWS.on("close", () => { clearInterval(rikIntervalCmd); setTimeout(connectRikWebSocket, 3000); });
    rikWS.on("error", (err) => { rikWS.close(); });
}

start().then(() => { connectRikWebSocket(); }).catch(err => { console.error('Failed to start application:', err); });
